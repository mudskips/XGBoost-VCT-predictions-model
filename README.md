...
The model saw decreased performance specifically at the two big LAN events, Masters Toronto and Champions, where it loses to the baseline (54.6% vs. 59.3%) even though it wins comfortably during the regional season (65.3% vs. 63.1%). See Limits below for why I think this might've happened.

## What the model uses

Round-margin momentum (how much a team has been winning/losing by, averaged over its last 5 maps) drives most of the model's decisions, aided by round win rates based on economy (pistol, eco, and full-buy rounds). No stats such as ADR, KAST, or any other player-level stat went into this model at all.
...
All data comes from Ryan Luong's VCT Kaggle
...
- Uses XGBoost (`max_depth=2`, heavy L1/L2 regularization, `min_child_weight=12`), which is shallow and regularized given the
